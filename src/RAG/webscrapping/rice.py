...
# TNAU rice sources. Each START_PAGE seeds a crawl; we recurse into any .html
# on the same host that carries a rice/paddy marker in its URL, so the scrape
# stays scoped to rice even across directories like /agriculture/ and
# /crop_protection/.
START_PAGES = [
    "http://www.agritech.tnau.ac.in/agriculture/"
    "agri_cropproduction_cereals%20index%20new_rice.html",
    "https://agritech.tnau.ac.in/crop_protection/"
    "crop_prot_crop%20diseases_cereals_rice_main.html",
]
ALLOWED_HOST_SUFFIX = "agritech.tnau.ac.in"
RICE_MARKERS = ("rice", "paddy")

visited = set()
pages_data = []

# Always write to src/RAG/rice_dataset/ regardless of where this is invoked.
output_folder = str(Path(__file__).resolve().parent.parent / "rice_dataset")
...
def should_follow(url: str) -> bool:
    """Same-host .html pages that carry a rice/paddy marker in the URL."""
    parsed = urlparse(url)
    ...
    if not url.lower().endswith(".html"):
        return False
    lowered = url.lower()
    return any(marker in lowered for marker in RICE_MARKERS)
